...
- 屏幕共享模式：适用于腾讯视频、抖音、B 站、爱奇艺、YouTube 等无法稳定嵌入或控制的第三方页面。房主在原网站正常播放和暂停，通过浏览器原生屏幕共享和 WebRTC 共享标签页画面与声音。
- 房主控制不依赖浏览器扩展。支持浏览器原生置顶管理台，房主切回原视频网页时仍可处理加入申请、聊天、成员和视频来源；不支持置顶管理台的浏览器会退回独立控制台窗口。
- 后端不存储、不下载、不转发影视资源，只负责临时房间、模式判断、WebSocket 状态同步、房主管理动作和 WebRTC 信令协调。
